...
public partial class ShowCart
{
    ...
    private async Task Delete(int temporalOrderId)
    {
        ...
        var responseHttp = await Repository.DeleteAsync($"api/temporalOrders/{temporalOrderId}"); //se quito <TemporalOrder> porque el metodo DeleteAsync no retorna nada, solo un HttpResponseWrapper con la respuesta del servidor, y en este caso no se necesita la respuesta del servidor, solo se necesita saber si hubo un error o no.
